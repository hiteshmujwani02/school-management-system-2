...
const socialLinks = [
  { href: "https://www.facebook.com/vigorousitsolutions/", label: "Facebook", icon: FaFacebook },
  { href: "https://x.com/vigorousit", label: "Twitter", icon: FaTwitter },
  { href: "https://www.instagram.com/vigorousitsolutions/", label: "Instagram", icon: FaInstagram },
  { href: "https://www.linkedin.com/company/vigorous-it-solutions/", label: "LinkedIn", icon: FaLinkedin },
  { href: "https://www.pinterest.com/vigorousitsolutions/", label: "Pinterest", icon: FaPinterest },
  { href: "https://www.youtube.com/@Vigorous-IT-Solutions", label: "YouTube", icon: FaYoutube },
];
              ...
              <div className="text-[16px] flex flex-col md:gap-2 gap-1">
                {navLinks.map(({ href, label }, index) => (
                 href ? <Link key={index} href={href} className="no-underline ">
                  ...
                  </Link> : <p key={index} className="!m-0 text-[16px] lg:text-[17px]">{label}</p>
                ))}
              </div>
            </div>
            <div className='space-y-5 flex-1'>
              ...
              <div className="text-[16px] flex flex-col md:gap-2 gap-1 h-[200px] overflow-auto ">
                ...
                href ?  <Link key={index} href={href} className="no-underline ">
                  ...
                  </Link>:      <p key={index} className="!m-0 text-[16px] lg:text-[17px]">{name}</p>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-3 ">
            ...
            <div className=" flex flex-col md:gap-2 gap-1">
              {navLinks.map(({ href, label }, index) => (
                ...
                </Link>) : (<p key={index} className="!m-0.5 ">{label}</p>)
              ))}
            </div>
          </div>
               ...
               {services.map((service, index) => (
                 service.href ? (
                   <Link key={index} href={service.href} className="no-underline text-white">
                     <p className="!m-0.5">{service.name}</p>
                   </Link>
                 ) : (
                   <p key={index} className="!m-0.5">{service.name}</p>
                 )
               ))}
             </div>
